            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-lg font-semibold">
                  {spot.available_slots ?? 0}/{spot.total_slots ?? 0}
                </p>
              </div>
              <div>
                <p className="text-sm text-slate-600">Price/Hour</p>
                <p className="text-lg font-semibold">${(spot.price_per_hour ?? 0).toFixed(2)}</p>
              </div>
            </div>

            <div className="flex items-center justify-between">
              <Badge className={getTrafficColor(spot.traffic_level || "Low")}>{spot.traffic_level || "Unknown"} Traffic</Badge>
              <span className="text-sm text-slate-600">Score: {((spot.scores?.final ?? 0) * 100).toFixed(1)}/100</span>
            </div>
